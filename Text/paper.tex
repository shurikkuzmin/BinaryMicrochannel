...
\address[google]{Google, Zurich, Switzerland}
...
for a bubble flow in liquid for a range of $Ca$ number. The binary
...
percent of the film thickness for simulation to give convergent results. While
the convergence criteria is fulfilled the results are in a good agreement with the
...
the interface using continuous interface methods where the interfaces spans over several grid nodes.
Such a representation
brings issues of the film thickness resolution versus interface
resolution. Thus, the diffusive interface should be resolved in such a way as to have
...
negligible Reynolds number effect on the film thickness for the relatively wide range of Reynolds
numbers.
...
binary liquid model, are good compared with other simulations
...
different ranges of parameters, simulations have been conducted and are typically validated with the
...
numbers using finite element method. \citet{giavedoni-numerical} performed cross validation of the
...
influence the film thickness results. This is obviously achieved only for certain relation of the
interface resolution related to the film thickness. The same applies to spurious
...
overall solution. Therefore, the diffusive interface should not noticeably affect the film
thickness.
...
bubble shape
is different in the front and rear menisci depending on Bond number ($Bo=\frac{\Delta \rho g
...
the initialization technique we used to determine parameters necessary for
...
\end{equation}
where $\mu_{\mathrm{liq}}$ is the liquid viscosity, and $\gamma$ is the interface tension. The
...
\end{aligned}
\end{equation}
The body force (\ref{poiseuille:velocity:center}) causes the difference  in the bubble
velocity from that implied by the capillary number, because of the bubble presence in a flow. That's
why to achieve the 
desired velocity and the
capillary number the iteration procedure is needed.
However, the initialization body force given by (\ref{poiseuille:velocity:center}) is a good
starting approximation. 

After one calculates the parameters necessary to run a simulation, it is necessary to
...
the convergence simulation time, because starting far away from the steady-state requires
...
$\tau_{\mathrm{liq}}=2.5$ and $\tau_{\mathrm{gas}}=0.7$, that correspond
...
grid size $400 \times 6000$. In terms of computer memory
...
computations for system to come to equilibrium. The simulation becomes longer with decrease of the
capillary number. Bretherton obtained the analytical solution for the range of low capillary
...
which
  ...
  The film thickness for such small Capillary numbers is taken from Fig. 
...
lattice units are related to the interface resolution.  Therefore the channel
  width is $200$ lattice units. Therefore, 
the employed grid size is $200 \times 3000$. This velocity can be
...
substituting the obtained velocity in the capillary number equation the
...
the grid resolution. To do that, all parameters are fixed,
including the bubble velocity, the capillary number, and only the grid
resolution is varied. Our goal is to determine the ratio of the interface thickness to the 
film thickness, at which results are not dependent on the grid resolution.
...
node and the fluid node. The location of the wall for simply hydrodynamics flows is
...
see that results converge for $H_{\mathrm{eff}}\geq 175$. However, one can see
...
attributed to the density and the viscosity ratios, which are much larger in the
...
of size $N_x\, \mathrm{x}\, N_y$. $U_{bubble}$ is the velocity of the bubble center measured at the
scaled to $H_{eff}$ $x=14.0$.
...
impose of wall gradients through the first-order finite difference scheme. Therefore, they do
...
small capillary numbers and make calculations for the range of capillary number $0.03-0.8$, which
is a computationally reasonable task. We choose consistent with the previous sections grid to be
$202 \times 3001$. $5$ lattice Boltzmann units do not occupy more than $60$
percent of the effective thickness. Because the simulation get unstable with
...
the computer resources constraints we are not able to provide simulations with long bubbles. Thus,
the qualitative comparison for the thickness variation is conducted in Fig.
...
did not find any significant differences with from the results presented above. The
...
we specifically designed the benchmark. The bubble was chosen sufficiently long for the film
thickness to
stabilize. Also, we used periodic boundary conditions for scheme robustness. Thus, the bubble
train was simulated instead of one bubble motion. To avoid the mutual influence of neighboring
bubbles a
distance between bubbles was chosen to be $3$ bubble sizes. The computational
results in terms of capillary number dependence and shape of a bubble show consistency with the
previously published data. Surprisingly, with large enough viscosity ratio the results are
independent on inertia. Also, we examined the influence of grid resolution on the results. We found
that if the phase interface is resolved as $50$ percent of the simulated film thickness, the
simulations
