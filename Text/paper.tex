...
proper film thickness resolution in comparison with the interface arises.  This work
...
density. While the classical Bretherton problem is stated for gas and liquid, which are of largely
...
resolutions of the interface in comparison with the film thickness. The same applies to spurious
...
\end{aligned}
\end{equation}
Even with this naive assumption for the $62.5$ percent resolution of the interface thickness of the
...
which
  ...
  The film thickness for such small Capillary numbers is taken from
...
resolution is changed. Our goal is to determine the ratio of the interface thickness to 
...
underresolved film thicknesses. To calculate how well the interface thickness is
...
to the grid refinement section where we established that the interface thickness should be
at most $50$ percent of the interface thickness.  We therefore choose the grid to be
...
\end{equation}
The film thickness is initialized through the capillary numbers ratio as well:
\begin{equation*}
