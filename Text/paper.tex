...
range of $0.05$ to $1.0$ where the capillary number is based on the liquid dynamic viscosity, the
...
as long as the liquid film thickness is at least twice the width of the diffuse interface, with
...
$\Re = 300$. They indicate that while the Reynolds number influence on the established film
...
with the experiments, the range of studied bubble lengths is limited to $2-3$ channel diameters.
...
overall solution. 
%Therefore, the diffuse interface should not noticeably affect the film
%thickness.
...
In these
examples we use $k=0.04$ and $A=0.04$ as numerically optimal binary liquid model parameters.
That implies the surface tension to be $\sqrt{\frac{8 k
A}{9}}=0.0377$ and the characteristic width of the interface
$5\sqrt{\frac{k}{A}}=5$ lattice Boltzmann units (grid spacings). For the sake of simplicity and
...
\end{equation}
where $\phi_0$ is the grid coordinate where the phase field is $0$, $N_y-2$
...
wall for the
multiphase models to the best of the authors' knowledge is not yet derived. Thus, the simulation
grid size
is $202\times 3000$. 
...
flow is close to planar Poiseuille flow with $U_{\mathrm{bubble}}$ being the maximun in the
Poiseuille profile:
\begin{equation}
...
$100000-300000$ time steps to reach steady-state, see Section \ref{sec:steady:state}.
...
velocity of the interface as a function of time. We can conclude that it is enough to have
$100000-300000$ iterations for
...
\hline
$100000$&$ 0.0655$&$0.0027$&$0.81$\\
\hline
$140000$&$0.0608$&$0.0027$&$0.81$\\
\hline
$180000$&$0.0577$&$0.0027$&$0.81$\\
\hline
$220000$&$0.0630$&$0.0027$&$0.81$\\
\hline
$260000$&$0.0660$&$0.0027$&$0.81$\\
\hline
$300000$&$0.0661$&$0.0027$&$0.81$\\
\hline
\end{tabularx}
...
%As it was pointed out above, the film thickness in that case is $6$ percent of the
%effective
...
\citet{heil-bretherton} that inertia effects influence insignificantly the film thickness at
...
bubble shape along with velocity pattern needs to be examined.}
...
it is enough for bubbles to have a length of two-three tube diameters to apply analysis for very
long
bubbles. In comparison with the axisymmetric geometry, for three-dimensional square shaped
...
having vortex in the front of the bubble for $\Ca=0.22$ and not having it for larger capillary
numbers
...
qualitative capture of the effects.  
}
\begin{figure}
