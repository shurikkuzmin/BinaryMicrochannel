...
range of $0.03$ to $0.8$ where the capillary number is based on the liquid’s dynamic viscosity, the
...
for the grid refinement, see Section \ref{section:grid:refinement}, we keep the same velocity
...
x } = \mathrm{const}$. For the designing capillary range simulations, we use a proportionality law
to initialize body forces knowing results of just one simulation:
\begin{equation}
\begin{aligned}
