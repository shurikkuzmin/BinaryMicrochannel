...
class GeoFE(geo.LBMGeo2D):
    ...
    def init_fields(self):
        ...
        self.sim.phi[np.logical_and(
                       ...
                       np.logical_and(hy >= width + 2 , hy <= self.lat_ny-width-3))] = -1.0
        self.sim.rho[:] = 1.0
...
class FESim(lbm.BinaryFluidFreeEnergy):
    ...
    def __init__(self, geo_class, defaults={}):
        settings = {'verbose': True, 'lat_nx': 2626,
                    'lat_ny': 177+2, 'grid': 'D2Q9',
        ...
        self.add_body_force((6.0e-6/(4.0*1.75*1.75), 0.0), grid=0, accel=False)
