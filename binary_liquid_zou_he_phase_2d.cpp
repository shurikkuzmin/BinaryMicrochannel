...
void general_pressure(int xCoor, int normx, int normy,double rho_pressure,double phase_pressure)
{
    ...
    for (int iY=1;iY<NY-1;iY++)
    {
        ...
        double phase_temp=phase[xCoor][iY];
        ...
        for(int iCount=0;iCount<unknown.size();iCount++)
        {
            ...
            double phasepos=weights[pos]*(phase_temp + 3.0*phase_temp*(cx[pos]*ux_temp+cy[pos]*uy_temp)
                            +4.5*phase_temp*((cx[pos]*cx[pos]-1.0/3.0)*ux_temp*ux_temp
                                             +(cy[pos]*cy[pos]-1.0/3.0)*uy_temp*uy_temp
                                             +2.0*ux_temp*uy_temp*cx[pos]*cy[pos]));
            ...
            double phaseneg=weights[neg]*(phase_temp + 3.0*phase_temp*(cx[neg]*ux_temp+cy[neg]*uy_temp)
                            +4.5*phase_temp*((cx[neg]*cx[neg]-1.0/3.0)*ux_temp*ux_temp
                                             +(cy[neg]*cy[neg]-1.0/3.0)*uy_temp*uy_temp
                                             +2.0*ux_temp*uy_temp*cx[neg]*cy[neg]));
            //Perform BB for the Non-equilibrium parts
            f[xCoor][iY][pos]=f[xCoor][iY][neg]+(eqpos-eqneg);
            g[xCoor][iY][pos]=g[xCoor][iY][neg]+(phasepos-phaseneg);
        }
        ...
        //Corrections for the phase field
        double phasex=0.0;
        double phasey=0.0;

        for(int iPop=0;iPop<9;iPop++)
        {
			phasex+=g[xCoor][iY][iPop]*cx[iPop];
			phasey+=g[xCoor][iY][iPop]*cy[iPop];
		}

        phasex=(phase_temp*ux_temp-phasex)/diagonal.size();
        phasey=(phase_temp*uy_temp-phasey)/diagonal.size();
        ...
        for(int iPop=0;iPop<diagonal.size();iPop++)
        {
            ...
            g[xCoor][iY][diagonal[iPop]] +=
               (!normx)*cx[diagonal[iPop]]*phasex
               +(!normy)*cy[diagonal[iPop]]*phasey;
...
// 					27.0/2.0*(cy[iPop]*cy[iPop]*cx[iPop]-1.0/3.0*cx[iPop])*uyeq*uyeq*uxeq);
            ...
            g2[xCoor][iY][iPop]=g[xCoor][iY][iPop]*(1.0-omega)+omega*geqeq[iPop];
        }
...
int main(int argc, char* argv[])
{
	...
	for(int counter=0;counter<=N;counter++)
	{
        ...
        general_pressure(0,1,0,rho_inlet,phase_inlet);
        general_pressure(NX-1,-1,0,rho_outlet,phase_outlet);
        collide_bulk();
        update_bounce_back();
